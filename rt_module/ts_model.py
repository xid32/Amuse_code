...
from htsat.layers import PatchEmbed, Mlp, DropPath, trunc_normal_, to_2tuple
...
def time_to_bs(tensor):
    bs = tensor.size(0)
    outs = []
    for i in range(bs):
        outs.append(tensor[i])
    outs = torch.cat(outs, dim=0)
    return outs

def bs_to_time(tensor, bs):
    outs = []
    inv = int(tensor.size(0) // bs)
    for i in range(bs):
        outs.append(tensor[i*inv:(i+1)*inv])
...
class Timestamp_Feature_Encoder(nn.Module):
    r"""TSTM
    Args:
    ...
    def __init__(self, spec_size=256, patch_size=4, patch_stride=(4, 4),
                 ...
                 use_checkpoint=False, for_avqa=False, norm_before_mlp='ln', config=None, **kwargs):
        super(Timestamp_Feature_Encoder, self).__init__()

        self.config = config
        self.for_avqa = for_avqa
        ...
        self.logmel_extractor = LogmelFilterBank(sr=config.sr, n_fft=config.window_size,
        ...
        self.load_swin_a_weight()


        self.swin_v = timm.create_model('swinv2_tiny_window8_256', pretrained=True)
        self.transformer_a = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(768, 2, 1024, 0.1, batch_first=True),
            2,
        )
        self.transformer_v = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(768, 2, 1024, 0.1, batch_first=True),
            2,
        )
        self.regressor = nn.Sequential(
            nn.Linear(768, 128),
            nn.GELU(),
            nn.Linear(128, int(self.config.max_sec // self.config.seg_during))
        )


    def load_swin_a_weight(self):
        ckpt = torch.load(self.config.swin_a_ckpt, map_location="cpu")
        ckpt_new = {}
        for i in ckpt["state_dict"].keys():
            ckpt_new[i.replace("sed_model.", "")] = ckpt["state_dict"][i]
        self.load_state_dict(ckpt_new, strict=False)
        return
    ...
    def forward(self, audio: torch.Tensor, video, mixup_lambda=None, infer_mode=False):  # out_feat_keys: List[str] = None):
        audio = rearrange(audio, 'b t s -> (b t) s')
        ...
        bs = video.size(0)
        f_v = rearrange(video, 'b t c h w -> (b t) c h w')
        f_v = self.swin_v.forward_features(f_v)
        f_v = bs_to_time(f_v, bs)
        # f_v = torch.split(f_v, bs, dim=0)
        # f_v = torch.stack(f_v, dim=0)
        f_v = torch.mean(f_v, dim=(2, 3))
        f_v = self.transformer_v(f_v)
        f_a = output_dict["latent_output"].float()
        f_a = bs_to_time(f_a, bs)
        f_a = self.transformer_v(f_a)

        # f_a = torch.split(f_a, bs, dim=0)
        # f_a = torch.stack(f_a, dim=1)

        if self.for_avqa:
            return f_v, f_a
        else:
            v_outs = self.regressor(torch.mean(f_v, dim=1))
            a_outs = self.regressor(torch.mean(f_a, dim=1))
            return v_outs, a_outs


# if __name__ == '__main__':
#     model = Timestamp_Feature_Encoder(config=configs).cuda()
#     a = torch.normal(0, 1, [2, 1920000]).cuda()
#     v = torch.normal(0, 1, [2, 20, 3, 256, 256]).cuda()
#     outs = model(a, v)
#     print(outs.shape)
