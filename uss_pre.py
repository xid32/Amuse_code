...
from uss_module.config import (ID_TO_IX, IX_TO_LB, LB_TO_IX, csv_paths_dict,
                        panns_paths_dict)
from uss_module.models.pl_modules import LitSeparation, get_model_class
from uss_module.models.query_nets import initialize_query_net
from uss_module.parse_ontology import Node, get_ontology_tree
from uss_module.utils import (get_audioset632_id_to_lb, get_path,
...
class Uss_batch:
    def __init__(
        ...
        self.query_emb_path = query_emb_path
...
def separate(args):
    ...
    audio_files = [os.path.join(audio_folder_path, file) for file in os.listdir(audio_folder_path) if
                   file.endswith('.wav')]
    # Initialize DataFrame to store BPM values for each audio
    df = pd.DataFrame(columns=['Audio'] + [f'Interval_{i + 1}' for i in range(num_intervals)])
    ...
    for audio_path in tqdm.tqdm(audio_files, total=len(audio_files)):
        ...
        # Initialize list to store counts values for each interval
        counts_intervals = []
        # Divide the audio into intervals and calculate counts for each interval
        for i in range(num_intervals):
            start_samples = i * segment_samples
            end_samples = (i + 1) * segment_samples
            # Extract audio segment for the current interval
            audio_interval = audio[start_samples: end_samples]
            if len(audio_interval) < segment_samples:
                counts_intervals.append(0)
                continue
            ...
            counts_intervals.append(counts)

        # Add audio name and counts values to DataFrame
        audio_name = os.path.basename(audio_path)
        df.loc[len(df)] = [audio_name] + counts_intervals
...
def separate_by_hierarchy(
    ...
    instruments_idxs = list(range(139, 216))
    counts = 0
        ...
        for node in nodes_level_n:
            ...
            if np.max(sep_audio) > non_sil_threshold:
                ...
                if label in LB_TO_IX.keys():
                    idx = LB_TO_IX[label]
                    if int(idx) in instruments_idxs:
                        counts += 1
...
def main(
    ...
    output_path = os.path.join(output_folder, 'source_separation.csv')
...
if __name__ == "__main__":
    audio_folder_path = "./datasets/audios"
    output_folder = './datasets/csv'
    levels = [3]
    class_ids = []
    config_yaml = "./uss_module/ss_model=resunet30,querynet=at_soft,data=full.yaml"
    checkpoint_path = "./uss_module/downloaded_checkpoints/ss_model=resunet30,querynet=at_soft,data=full,devices=8,step=1000000.ckpt"
    main(audio_folder_path, output_folder, levels, class_ids, config_yaml, checkpoint_path)
